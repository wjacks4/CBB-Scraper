...
playerinfo=pd.read_csv('D:/WebscrapeInfo/BryanStuffFull.csv', header=1, names=['index', 'player', 'career', 'fulldata'])
...
for eachplayer in namelistcat:
			...
			for num in numlist:
				...
				try:
					...
					for comment in comments:
						...
						if table is not None:
							...
							tabledata.to_csv('D:/WebscrapeInfo/DataByPlayer/%s%s.csv' %(player,titlecode))
					
							access='D:/WebscrapeInfo/DataByPlayer/%s%s.csv' %(player, titlecode)
							#print(access)
				...
				dfstart.to_csv('D:/WebscrapeInfo/PlayerTotals/%sTOTAL.csv' %(player))
